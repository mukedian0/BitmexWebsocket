...
module.exports = {
  ...
  _partial(tableName, symbol, client, data) {
    ...
    return {"action": "init", data: dataArr};
  },
...
function insertIntoStore(context, key, newData) {
  ...
  //add by hgr: return update data, full data can get by getData method
  //return replaceStore(context, key, storeData);
  replaceStore(context, key, storeData);
  return {"action":"insert", data: newData};
  //end
}
...
function updateStore(context, key, newData, keys) {
  ...
  //return replaceStore(context, key, storeData);
  replaceStore(context, key, storeData);
  return {"action": "update", data: updateData};
  //end
}
...
function removeFromStore(context, key, newData, keys) {
  ...
  for (let i = 0; i < newData.length; i++) {
    ...
    removeData.push(itemToRemove);
  }

  //add by hgr: return update data, full data can get by getData method
  //return replaceStore(context, key, storeData);
  replaceStore(context, key, storeData);
  return {"action":"remove", "data": removeData};
  //end
}
